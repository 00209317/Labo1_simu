...
float inversa(Matrix invert, Matrix &Inversa)
{
    Matrix deter_invert, copy_inver, copy_cofactors, answer_matrix;

    zeroes(Inversa, invert.size());

    cout << "Su determinante es" << endl;
    float det = determinant(invert);
    cout << det << endl;
    if (det == 0)
        exit(EXIT_FAILURE);
    ...
    cout << endl;
    //showMatrix(invert);
    transpose(invert, copy_inver);
    //cout << endl;
    //showMatrix(copy_inver);
    //cout << endl;
    cofactors(copy_inver, copy_cofactors);
    //cout << endl;
    //showMatrix(copy_cofactors);
    cout << "Su respuesta es" << endl;
    productRealMatrix(1 / det, copy_cofactors, answer_matrix);
    //showMatrix(answer_matrix);
    cout << endl;

    for (int i = 0; i < answer_matrix.size(); i++)
        for (int j = 0; j < answer_matrix.at(0).size(); j++)
            Inversa.at(i).at(j) = answer_matrix.at(i).at(j);
}
